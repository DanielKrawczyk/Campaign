...
function Story() {
    ...
    (function addNewStory() {
        switch(story.type) {
            ...
            case 'visit':
                ...
                break;
            case 'gameover_jail':
                text = <p>&emsp;{data.gameover.jail}<br /><br />[ Nić przeznaczenia jest bardzo krucha. Ismena nie może trafić do więzienia! ]</p>
                next = <a href="#story" onClick={() => {sounds.click.play();window.location.reload()}}>Powrót do menu głównego.</a>;
                break;
            default:
                text = '';
                next = '';
    ...
    function handleCheckRequirements(data) {
        ...
        switch(require.type) {
            case 'item':
                ...
                if (item) return addTextToButton(text, path);
                else return '';

            case 'quest':
                quest = store.getState().quests.find(f => f.id === require.id);
                if (quest) return addTextToButton(text, path);
                else return '';
            ...
            case 'person_visited':
                person = store.getState().visited.find(f => f.id === require.id);
                if (person.visited === true) return addTextToButton(text, path);
                else return '';

            case 'person_not_visited':
                person = store.getState().visited.find(f => f.id === require.id);
                if (person.visited === false) return addTextToButton(text, path);
                else return '';
    ...
    function addTextToButton(text, path) {
        return (
            <li key={text}>
